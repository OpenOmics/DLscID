# <code>DLscID <b>unlock</b></code>

## 1. About 
The `DLscID` executable is composed of several inter-related sub commands. Please see `DLscID -h` for all available options.

This part of the documentation describes options and concepts for <code>DLscID <b>unlock</b></code> sub command in more detail. With minimal configuration, the **`unlock`** sub command enables you to unlock a pipeline output directory. 
...
Unlocking DLscID pipeline output directory is fast and easy! In its most basic form, <code>DLscID <b>unlock</b></code> only has *one required input*.

## 2. Synopsis
```text
$ ./DLscID unlock [-h] --output OUTPUT
```
...
DLscID unlock --output /data/$USER/output
```